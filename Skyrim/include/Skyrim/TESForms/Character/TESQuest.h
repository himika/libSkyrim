...
#include "../../Radiant Story/BGSStoryManagerTreeForm.h"
...
public:
	...
	///<summary>Obtains whether the specified quest objective is completed or not.</summary>
	bool IsObjectiveCompleted(UInt32 index) const {
		Objective *objective = GetObjective(index);
		return (objective && objective->IsCompleted());
	}
	///<summary>Obtains whether the specified quest objective is displayed or not.</summary>
	bool IsObjectiveDisplayed(UInt32 index) const {
		Objective *objective = GetObjective(index);
		return (objective && objective->IsDisplayed());
	}

	DEFINE_MEMBER_FN(		SetActive, void, 0x0056B530);
	DEFINE_MEMBER_FN_const(	GetObjective, Objective *, 0x0056BBB0, UInt32 index);
